...
typedef union Value {float fVal; int iVal; void *mVal;} Value;
typedef enum Typename {INT_T, FLOAT_T, ARRAY_T} Typename;
typedef struct {Value value; Typename type;} SymbolInfo;
typedef struct {SymbolInfo info; char *name;} Symbol;
typedef struct strLink {Symbol symbol; struct strLink *next;} Link;
typedef struct {Link *head; Link *tail;} SymbolList;
typedef struct {SymbolList **sl; size_t size;} SymbolTable;
